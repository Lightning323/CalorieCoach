...
class CoachAIService {
  ...
  async promptPiece(text: string, foodItems?: FoodItemAI[], addOpenFoodFacts = true): Promise<{ prompt: string, allMatches: FoodItem[] }> {
    ...
    if (foodItems) {
      ...
      if (addOpenFoodFacts) {
        ...
        const results2Promise = await OpenFoodFactsApi.getAPIFoodMatches(foodItemString_api, 3);
      ...
      foodItems.forEach((foodItem, i) => {
        ...
        if (results2[foodItem.name] && results2[foodItem.name].length > 0) {
          if (firstTime2) {
            prompt += "Internet searches (Use these only as a reference for new entries):\n";
            prompt += "name,\t quantity,\t calories\n";
            firstTime2 = false
          } else {
            prompt += "Internet searches:\n";
          }
      ...
      (await FoodDatabase.searchFoods(text, 8)).forEach((match) => allMatches.push(match.item))
  ...
  async logFood(username: string, foodItemsText: string, simpleMode: boolean): Promise<String> {
    ...
    try {
      ...
      console.log("Logging food... Simple mode: ", simpleMode);
      if (simpleMode) {
        var { prompt, allMatches } = await this.promptPiece(foodItemsText.replace(/"/g, '\\"').toLowerCase())
      } else {
        try {
          ...
          console.log("Error getting individual food items: ", e);
          simpleMode = true
          var { prompt, allMatches } = await this.promptPiece(foodItemsText.replace(/"/g, '\\"').toLowerCase())
        }
      }
...
- quantity = multiplier (e.g., 2x a 1-cup item = double calories)
- If food exists, include "match_id"
- If food is new, omit "match_id" and include "new_food"

format:
[{ "quantity": number, "match_id": number },
{
  "quantity": number,
